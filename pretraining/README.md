...
Use the frozen released embedding file with the released checkpoints. Regeneration is intended for retraining; embeddings of proteins longer than 1,024 residues can differ from the historical run because the old script split them only after a GPU out-of-memory error.
