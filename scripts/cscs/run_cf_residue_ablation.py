...
def settings(root):
    return yaml.safe_load((root / "source/configs/cf_residue_ablation.yaml").read_text())
...
def train_command(root, trial, *, smoke_name=None):
    ...
    args += ["--epochs", str(4 if smoke_name else config["epochs"]),
             ...
             "--decoder-dropout", "0", "--mask-ratio", "0.5", "--mask-type", "dm",
             "--k-negatives", "1"]
    if trial["mode"] != "mean":
...
def prepare(root, previous):
    ...
    grouped = [[t for t in trials if t["mode"] == mode] for mode in MODES]
    ...
    queues = {
        "smoke": [dict(key=m, command=command("smoke_one", m)) for m in MODES],
        "train": [dict(key=t["name"], command=command("train_one", i)) for i, t in enumerate(trials)],
        "evaluate": [dict(key=m, command=command("evaluate_one", m)) for m in ["bos", *MODES]],
        "downstream": [dict(key=f"{m}_{t['task']}", command=downstream_command(root, m, t))
                       for m, t in itertools.product(["bos", *MODES], inputs["tasks"])],
    }
    ...
    save_json(root / "PREPARED.json", dict(pretraining_runs=len(trials), downstream_configs=408,
                                          downstream_queue_tasks=len(queues["downstream"])))
    print(f"Prepared {len(trials)} CF training configurations; 408 downstream configurations", flush=True)
...
def select(root):
    ...
    save_json(root / "selected.json", {m: max((r for r in rows if r["mode"] == m), key=lambda r: r["score"]) for m in MODES})
...
def summarize(root):
    ...
    for task in read_json(root / "inputs.json")["tasks"]:
        ...
        assert len(files) == 24, f"Incomplete downstream configs for {task['task']}: {len(files)}/24"
    ...
    (root / "RESULTS.md").write_text("# CF residue pooling ablation\n\nAUPRC (%). Same released task cohorts/folds; localization is the preserved 37-label development benchmark. CF poolers are learned on global PPI only; independent ESM poolers are freshly learned on downstream training folds.\n\n" + overview.round(2).to_markdown(index=False) + "\n")
...
def submit(root, stage="smoke", allocation=1):
    ...
    job = subprocess.check_output(["sbatch", "--parsable", "--job-name", f"cf-pool-{stage}",
