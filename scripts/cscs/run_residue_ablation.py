...
def grid(config):
    ...
    for mode in MODES:
        ...
        for lr, width, rate in itertools.product(config["backbone_lr"], widths, rates):
            ...
            result.append(dict(name=name, mode=mode, backbone_lr=lr, hidden_dim=width, lr=rate))
    return result
...
def sequence_command(root, mode, task):
    ...
    if mode != "bos":
        replace_arg(command, "--esm2-embeddings", root / "sequence" / f"{mode}.plk")
    return command
...
def prepare(root, release):
    ...
    # Four short runs cover both learned poolers and use all four GPUs.
    smoke = [next(t for t in trials if t["mode"] == mode) for mode in MODES] + [trials[-1]]
...
def summarize(root):
    ...
    for mode, task in itertools.product(["bos", "mean"], inputs["tasks"]):
        ...
        rows.append(dict(pooling=mode, task=task["task"], readout=f"LR_ESM_{mode}",
